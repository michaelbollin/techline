#!/usr/bin/env npx tsx
/**
 * Seed timeline images for events 71–120 using paced Wikipedia lookups,
 * Openverse fallback, and license checks before download.
 *
 * Usage:
 *   npx tsx scripts/seed-media-batch-3.ts [offset] [limit] [--dry-run]
 */
...
const offset = Number(process.argv[2] ?? 70);
...
async function main() {
  ...
  for (const event of batch) {
    ...
    const wikiTitle = BATCH_3_WIKI_TITLES[event.id];
    if (!wikiTitle) {
    ...
    const commonsOverride = BATCH_3_COMMONS_OVERRIDES[event.id];
    ...
    if (!resolved) {
      const openverseQuery =
        BATCH_3_OPENVERSE_QUERIES[event.id] ?? `${wikiTitle} computer history`;
